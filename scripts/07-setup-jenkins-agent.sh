...
echo "  3) On Jenkins: create credential 'id-creds-aws' (AWS Credentials) with IAM user/keys that have ECR + EKS access."
echo "  4) Jenkins credential 'ecr-registry-url' (Secret text) = terraform output ecr_registry_host (hostname only)."
echo "  5) For infra-as-code: run scripts/00-bootstrap-tf-backend.sh once, then create Jenkins job (Script Path jenkins/Jenkinsfile.infra)."
